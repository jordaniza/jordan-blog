...
In the last part, we attempted to solve the MagicNumber contract, by writing raw bytecode directly to an address using a foundry cheatcode. In this article, we remove that dependency, and create the contract ourselves.
...
We've been using foundry's `vm.etch` cheatcode to write contract data to an address. We can hardly call ourselves pro-hackers if we need to artificially manipulate the EVM state to win.
